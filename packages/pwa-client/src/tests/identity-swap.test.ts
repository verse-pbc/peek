...
import { generateSecretKey, getPublicKey, finalizeEvent, type EventTemplate, nip19 } from 'nostr-tools';
...
describe('Identity Migration Functionality', () => {
  ...
  beforeEach(() => {
    ...
    // Store auto-generated identity (unified single key)
    const autoIdentity = {
      type: 'local',
      secretKey: bytesToHex(autoGeneratedSk),
      publicKey: autoGeneratedPk,
      npub: nip19.npubEncode(autoGeneratedPk),
      createdAt: Date.now()
    };
    localStorage.setItem('peek_nostr_identity', JSON.stringify(autoIdentity));
  });

  describe('Auto-generated Identity Creation', () => {
    it('should create local identity', () => {
      const stored = JSON.parse(localStorage.getItem('peek_nostr_identity') || '{}');
      expect(stored.type).toBe('local');
  ...
  describe('Identity Storage', () => {
    it('should switch from local to extension identity', () => {
      // Verify local identity exists (unified storage)
      const localStored = JSON.parse(localStorage.getItem('peek_nostr_identity') || '{}');
      expect(localStored.type).toBe('local');

      // Switch to extension identity
      const extensionIdentity = {
        type: 'extension',
        publicKey: realIdentityPk,
        npub: nip19.npubEncode(realIdentityPk),
      };
      localStorage.setItem('peek_nostr_identity', JSON.stringify(extensionIdentity));

      // Verify switch (different type)
      const stored = JSON.parse(localStorage.getItem('peek_nostr_identity') || '{}');
      expect(stored.publicKey).toBe(realIdentityPk);
      expect(stored.type).toBe('extension');
    });
