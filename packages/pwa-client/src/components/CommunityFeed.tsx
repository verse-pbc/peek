...
export function CommunityFeed({
  ...
  const { identity, isAutoGenerated } = useNostrLogin();
      ...
      >
        {loading ? (
          <div className="flex items-center justify-center h-full text-muted-foreground">
            {t('community_feed.loading')}
          </div>
        ) : messages.length === 0 ? (
          <div className="flex items-center justify-center h-full text-muted-foreground">
            {t('community_feed.empty')}
          </div>
        ) : (
                      ...
                      <div className={`flex-1 min-w-0 ${isOwnMessage ? 'flex flex-col items-end' : ''}`}>
                        <div className={`flex items-baseline gap-1 mb-1 ${isOwnMessage ? 'flex-row-reverse' : ''}`}>
                          <UserProfile
                            ...
                            groupId={groupId}
                          />
                          {isOwnMessage && isAutoGenerated && (
                            <span className="text-xs text-muted-foreground/60 italic">
                              {t('community_feed.you')}
                            </span>
                          )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-background border-t p-4 z-50 shadow-lg">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex-1 min-w-0">
            <MentionInput
              ...
              placeholder={t('community_feed.input_placeholder')}
        ...
        {!identity && (
          <p className="text-xs text-muted-foreground mt-2">
            {t('community_feed.not_logged_in')}
          </p>
        )}
        </div>
      </div>
